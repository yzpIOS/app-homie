...
class HomeActivityBannerView extends StatefulWidget {
  const HomeActivityBannerView({super.key});

  @override
  State<HomeActivityBannerView> createState() => _HomeBannerViewState();
}

class _HomeBannerViewState extends State<HomeActivityBannerView> with GetStateMixin {
  ...
  late final ctrl = Get.find<ActivityBannerCtrl>();
  ...
  Widget itemBuilder(Map data) {
    ...
    child = OpacityButton(
      child: child,
      onTap: () {
        Get.to(() => WebViewPage(title: data['title'], url: data['url']));
      },
    );
